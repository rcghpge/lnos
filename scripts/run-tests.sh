...
run_static_analysis() {
    ...
    if set -o pipefail && shellcheck "$PROJECT_ROOT/scripts/LnOS-installer.sh" 2>&1 | tee /tmp/shellcheck.log; then
...
run_e2e_tests() {
    ...
    log_warn "E2E tests take 30+ minutes. Run manually with: ./test/e2e-tests.sh"
		if bash "$PROJECT_ROOT/test/e2e-tests.sh"; then 
        log_info "✓ e2e tests passed"
        record_result "e2e_tests" "pass"
		else
        log_error "✗ e2e tests failed"
        record_result "e2e_tests" "fail"
		fi
    record_result "e2e_tests" "skip"
}
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --integration-only)
                ...
                ;;
            *)
                echo "Usage: $0 [--static-only|--unit-only|--e2e]"
                echo ""
                echo "Options:"
